...
2026/03/29
* インターフェイスを見直して、より編集しやすくしたつもり
* SVGとPNGの出力解像度を向上(約2倍)、表示する地物を見直しました
* ChatGPTに海岸線処理の見直しを指示するも、まだ微妙な感じ
* Overpass APIのエラー処理を多少改善(エラー時はリトライ)

2026/04/05
* サーキット、ローラーコースターを表示するように追加
* インターフェイスを少し修正（スクロールやはみ出し時の処理）
* 正しく検索されるよう、meta descriptionを追記
* スマホ対応を少しだけ追加
* pedestrianがlineなのにpolygon描画される不具合を修正

2026/08/15
* 海岸線処理を見直し、港湾部などで陸地が欠けたり海として描画されたりする問題を改善
* 地図の背景色と各カラーパレットを見直し
* POIアイコンを`icon`フォルダへ集約し、PNG/SVGの参照処理を更新
* POIのアイコン変更画面で、設定済みの全アイコンを選択できるよう改善
* 地図へ追加できるPOIアイコンを最大100件に制限し、選択画面にエラーを表示
* SVG/PNGを手書き風に出力する機能を追加
* Roughness、Bowing、Fill Styleを変更でき、地図上で手書き風のプレビューを確認可能
* 手書き変換時もPOIアイコン、名称、画像、著作権表記を通常表示のまま維持
* Google AnalyticsのCookieドメイン設定をユーザー設定ファイルへ移動
* 画面サイズ変更時に手書き風プレビューが地図からはみ出す問題を修正
* 地図レイヤーでは`level`が負数の地下地物を除外し、POIは階層にかかわらず表示対象に維持
* 手書き風設定を他の編集項目と同じデザインへ統一し、設定名を日本語化
* 「マップを作る」ボタンを強調し、検索ボタンを通常表示へ変更
* ズームレベルに応じたSVG線幅の計算を調整
* OG画像を更新し、SNS共有用メタデータを変更
* `icon`フォルダへの移行に伴い、旧`maki-icon`フォルダを削除

2026/08/24
* PCとスマートフォンの基本メニューにドラッグハンドルを追加し、地図に重ねたまま幅・高さを変更できるよう改善
* 基本メニューをリサイズしても地図の表示範囲とPNG/SVGの出力開始位置が変わらないよう修正
* PNG/SVG出力時の線幅を出力倍率に合わせ、道路や建物の見た目が画面表示と揃うよう修正
* 道路のセンターラインとズームレベルに応じた線幅を調整
* レール類を濃色の線と明色の破線を重ねた表現へ変更
* OSMの`layer=*`を画面表示とPNG/SVG出力の描画順へ反映
* `barrier=kerb`と`man_made=embankment`を壁・擁壁レイヤーの表示対象へ追加
* PNG/SVGの作成中に処理中画面を表示し、連続操作を防ぐよう改善
...
2026/09/13
* 日本語ページと英語ページを分離し、`/en/`に英語版を追加
* ページの表示言語に合わせてタイトル、説明、OGP、構造化データ、画面内表示を切り替えるよう改善
* DataTablesを3.0.3、Selectを4.0.1へ更新し、日英の言語設定を現行形式へ移行
* Bootstrapを5.3.8、jQueryを3.7.1へ更新
* その日の初回アクセス時に、まち歩きマップメーカーの紹介を表示する機能を追加
* 紹介は閉じるボタンまたは「マップを作る」ボタンの押下時に閉じ、同じ日は再表示しないよう対応
* 低ズーム時も「マップを作る」ボタンを表示し、必要なズームへ達するまでは無効状態で案内するよう変更
* ズームすると追加される地物を一覧表示し、一覧が長い場合は縦スクロールできるよう改善
* 基本メニューを小さくした際も、ドロップダウンメニューが地図上で途切れないよう修正
* 基本メニューのボタン配置とラベルを見直し、ヘッダーの副題を上下中央へ調整
* モーダルを閉じる際のフォーカス処理と`inert`属性を追加し、アクセシビリティ警告を解消
* 住所が空欄のときは検索処理を実行しないよう修正

2026/09/14
* レイヤーの表示・非表示と線幅変更時に対象レイヤーだけを更新し、操作の待ち時間を短縮
* 時間のかかるレイヤー更新中に、アニメーションと背景暗転のない小さな処理中画面を表示
* マップ作成時のレイヤー生成を分割して画面表示を更新し、全レイヤー生成後に描画順を一度だけ適用するよう改善
* レイヤー一覧へ「項目・表示・色・線幅」の列見出しを追加
* 基本メニューのドラッグハンドルを廃止し、最小化・拡大・復元ボタンを備えたウインドウ形式へ変更
* 基本メニューの内容が高さを超えた場合に縦スクロールできるようにし、リセット後も最小化時の高さを維持
* 樹木と入口の半径、輪郭線幅、輪郭色、塗り色を`data/leyers.jsonc`の`pointStyle`で設定可能に変更
* `pointStyle`が未定義のPoint地物は基本レイヤーへ描画せず、個別追加したPOIアイコンは従来どおり表示
* `drawOrder`設定を追加し、同じOSM `layer=*`内では施設敷地より池・川岸・水路を前面へ描画
* 住所検索ボタンを虫眼鏡アイコンへ変更し、検索入力欄を拡張
